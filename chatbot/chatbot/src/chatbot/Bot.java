...
public class Bot {
	...
	static private String projID = "canvas-bot-346704";
	...
	public static String generateResponse(String s) throws Exception {
		...
		if (s.toLowerCase().contains("translate")) {
	...
	public static String translation(String s) {
		...
		try {
			...
			ar.add("https://www.googleapis.com/auth/cloud-platform");
			File f = new File("../../bin/canvas-bot-346704-1e59824eea32.json");
			GoogleCredentials credentials = GoogleCredentials.fromStream(new FileInputStream(f)).createScoped(ar);
		...
		for (int i = 0; i < words.length; i++) {
			if (words[i].toLowerCase().equals("translate")) {
				start = i+1;
			} else if (words[i].toLowerCase().equals("to")) { // can you translate _____ to french?
				end = i;
				for (int j = 0; j < languages.length; j++) {
					if (words[i+1].toLowerCase().equals(languages[j][1])) {
						langCode = languages[j][2];
						j = j+10;
					}
				}
			...
			} else if (words[i].toLowerCase().equals("from")) {
				return "Sorry could you repeat that? I only need to know what language you would like translated to.";
			}
		}
		...
		try (TranslationServiceClient client = TranslationServiceClient.create()){
			LocationName parent = LocationName.of(projID, "us-west3");
			TranslateTextRequest req = TranslateTextRequest.newBuilder().setParent(parent.toString()).setMimeType("text/plain")
					.setTargetLanguageCode(langCode).addContents(sentence).build();
			TranslateTextResponse response = client.translateText(req);
			return "Your sentence: "+sentence+", was translated to: "+response.getTranslationsList();
		} catch (Exception e) {
			e.printStackTrace();
		}
		
		return "Sorry, something went wrong with the translator";
	}
